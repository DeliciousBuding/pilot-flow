...
def test_project_entry_card_for_initial_risk_uses_red_header():
    chat_id = "oc_entry_card_risk_header"
    captured_cards = []

    def fake_send_card(_chat_id, card):
        captured_cards.append(card)
        return "om_entry_risk_header"
    ...
    assert result["status"] == "project_space_created"
    assert captured_cards[0]["header"]["template"] == "red"
...
def test_direct_card_action_retryable_failure_keeps_action_ref_for_briefing_batch_reminder():
    with _project_registry_lock:
        _project_registry.clear()
    with _plan_lock:
        _card_action_refs.clear()
    overdue = (dt.date.today() - dt.timedelta(days=1)).isoformat()
    _register_project(
        "直调批量重试催办项目", ["张三"], overdue, "进行中", [],
        goal="验证批量催办失败后可重试", deliverables=["验收记录"],
    )
    action_id = _create_card_action_ref(
        "oc_direct_retry_batch_reminder",
    ...
    def fail_then_send(chat_id, msg):
        send_attempts.append((chat_id, msg))
        return len(send_attempts) > 1

    with (
        patch("tools._hermes_send", side_effect=fail_then_send),
    ...
    assert "批量催办发送失败" in first["error"]
    assert second["status"] == "briefing_batch_reminder_sent"
    assert second["projects"] == ["直调批量重试催办项目"]
...
def test_update_project_remove_member_requires_explicit_confirmation_without_writes():
    with _project_registry_lock:
        _project_registry.clear()
    _register_project(
        "确认移除成员项目", ["张三", "李四"], "2026-05-20", "进行中",
        ["文档: https://example.invalid/doc"],
        app_token="app1", table_id="tbl1", record_id="rec1",
        goal="验证移除成员确认", deliverables=["验收记录"],
    )
    ...
    with (
        ...
        result = json.loads(_handle_update_project(
            {"project_name": "确认移除成员", "action": "remove_member", "value": "李四"},
            chat_id="oc_remove_member_gate",
        ))

    assert result["status"] == "confirmation_required"
    assert result["action"] == "remove_member"
    assert "移除成员" in result["reason"]
    update_bitable.assert_not_called()
...
def test_dashboard_followup_task_adds_public_update_after_restart(tmp_path):
    state_path = tmp_path / "pilotflow-projects.json"
    with _project_registry_lock:
        _project_registry.clear()
    deadline = (dt.date.today() - dt.timedelta(days=1)).isoformat()
    with patch.dict(os.environ, {"PILOTFLOW_STATE_PATH": str(state_path)}):
        ...
        with (
            ...
            result = json.loads(_handle_card_action({"action_value": action_value}, chat_id="oc_restart_dashboard_followup"))
        projects = _load_project_state()
    ...
    create_task.assert_called_once_with(
        ...
        "",
        deadline,
        "oc_restart_dashboard_followup",
        [],
    )
    append_doc.assert_called_once()
    restored_project = append_doc.call_args.args[1]
    assert "文档: https://example.invalid/docx/doc_dashboard_followup_restart" in restored_project["artifacts"]
    append_history.assert_not_called()
    assert sent_messages
...
def test_risk_project_detail_card_offers_reminder_button():
    with _project_registry_lock:
        _project_registry.clear()
    with _plan_lock:
        _card_action_refs.clear()
    _register_project(
        "详情风险催办项目", ["张三"], "2026-05-20", "有风险", [],
        goal="验证风险详情催办", deliverables=["风险处置记录"],
    )
    captured = {}
    action_value = _opaque_card_action_value("oc_detail_risk_reminder", "project_status", {"title": "详情风险催办项目"})

    def capture_card(chat_id, card):
        captured["card"] = card
        return "om_detail_risk_reminder"

    with patch("tools._hermes_send_card", side_effect=capture_card):
        result = json.loads(_handle_card_action({"action_value": action_value}, chat_id="oc_detail_risk_reminder"))

    assert result["status"] == "project_status_sent"
    actions = [element for element in captured["card"]["elements"] if element.get("tag") == "action"]
    assert actions
    button_text = [button["text"]["content"] for button in actions[0]["actions"]]
    assert button_text == ["解除风险", "发送提醒", "创建待办"]
    reminder_value = actions[0]["actions"][1]["value"]
    assert "pilotflow_action_id" in reminder_value
    assert "pilotflow_chat_id" not in reminder_value
    with _plan_lock:
        refs = [ref for ref in _card_action_refs.values() if ref["chat_id"] == "oc_detail_risk_reminder"]
    assert {ref["action"] for ref in refs} == {"resolve_risk", "send_project_reminder", "create_followup_task"}
...
def test_direct_card_action_retryable_failure_keeps_action_ref_for_followup_task():
    with _project_registry_lock:
        _project_registry.clear()
    with _plan_lock:
        _card_action_refs.clear()
    _register_project(
        "直调重试待办项目", ["张三"], "2026-05-10", "进行中", [],
        goal="验证直调失败后可重试", deliverables=["验收记录"],
    )
    action_id = _create_card_action_ref(
        "oc_direct_retry_followup",
        "create_followup_task",
        {"title": "直调重试待办项目"},
    )

    with (
        patch("tools._create_task", side_effect=[None, "直调重试待办项目跟进"]),
        patch("tools._hermes_send", return_value=True),
        patch("tools._append_project_doc_update", return_value=True),
        patch("tools._append_bitable_update_record", return_value=True),
    ):
    ...
    assert "待办创建失败" in first["error"]
    assert second["status"] == "project_followup_task_created"
    with _plan_lock:
        assert action_id not in _card_action_refs
...
def test_direct_card_action_retryable_failure_keeps_action_ref_for_dashboard_followup_task():
    ...
    with _plan_lock:
        _card_action_refs.clear()
    _register_project(
        "直调看板重试待办项目", ["张三"], "2026-05-10", "进行中", [],
        goal="验证看板直调失败后可重试", deliverables=["验收记录"],
    )
    action_id = _create_card_action_ref(
        "oc_direct_retry_dashboard_followup",
        "project_followup_task",
        {"title": "直调看板重试待办项目"},
    )

    with (
        patch("tools._create_task", side_effect=[None, "直调看板重试待办项目跟进"]),
        patch("tools._hermes_send", return_value=True),
        patch("tools._append_project_doc_update", return_value=True),
        patch("tools._append_bitable_update_record", return_value=True),
    ):
