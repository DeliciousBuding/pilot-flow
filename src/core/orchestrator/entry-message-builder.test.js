...
const html = buildProjectAnnouncementHtml({
  runId: "run-entry",
  plan: { goal: "Launch <PilotFlow> MVP" },
  artifacts: [
    {
...
assert.match(html, /PilotFlow 项目入口/);
assert.match(html, /Run ID/);
assert.match(html, /Launch &lt;PilotFlow&gt; MVP/);
assert.doesNotMatch(html, /Launch <PilotFlow> MVP/);
