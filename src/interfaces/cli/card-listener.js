...
function loadListenerConfig() {
  ...
  return {
    ...
    inputPath: resolve(args.input || process.env.PILOTFLOW_INPUT_PATH || "src/interfaces/cli/fixtures/demo_input_project_init.txt"),
...
function printUsage() {
  console.log(`Usage:
  npm run pilot:listen
  npm run pilot:listen -- --dry-run
  npm run pilot:listen -- --dry-run --max-events 1 --timeout 30s
  npm run pilot:listen -- --profile pilotflow-contest --chat-id oc_xxx --base-token xxx --base-table-id tbl_xxx
