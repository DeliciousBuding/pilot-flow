...
import type { DetectedRisk, RiskDecisionSummary } from "./risk.js";

export const RISK_DECISION_CARD_TYPE = "risk_decision";
export const RISK_DECISION_ACTIONS = ["assign_owner", "adjust_deadline", "accept_risk", "defer"] as const;

export interface BuildRiskDecisionCardOptions {
  readonly runId: string;
  readonly plan: Pick<ProjectInitPlan, "goal">;
  readonly risks: readonly DetectedRisk[];
  readonly summary: RiskDecisionSummary;
}

export function buildRiskDecisionCard({ runId, plan, risks, summary }: BuildRiskDecisionCardOptions): Record<string, unknown> {
  return {
    config: {
...
function decisionButton(text: string, action: (typeof RISK_DECISION_ACTIONS)[number], type: string, runId: string): Record<string, unknown> {
  return {
    tag: "button",
    text: {
      tag: "plain_text",
    ...
    type,
    value: {
      pilotflow_card: RISK_DECISION_CARD_TYPE,
...
function formatTopRisks(risks: readonly DetectedRisk[]): string {
...
function templateForRisk(level: string): string {
