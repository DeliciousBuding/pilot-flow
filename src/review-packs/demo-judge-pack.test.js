...
try {
  const files = {
    ...
    roadmap: await writeTemp("ROADMAP.md", "## Phase 3: Productization Cleanup"),
    playbook: await writeTemp("DEMO_PLAYBOOK.md", "6 to 8 minute script"),
    failurePaths: await writeTemp("FAILURE_PATHS.md", "Fallback map"),
  ...
  assert.match(markdown, /npm run review:judge/);
} finally {
