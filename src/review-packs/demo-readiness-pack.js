...
const DEFAULT_DOC_FILES = [
  ...
  },
  {
    ...
    anchor: "Review packs",
    purpose: "Compact index and reviewer boundary language."
  }
];
...
function buildNextActions({ status }) {
  if (status !== "ready_for_manual_capture") {
    return [
      "Regenerate missing evidence review before recording.",
      "Open the generated readiness report and fix missing anchors.",
      "Rerun `npm run review:readiness` until required files are ready."
    ];
  }
