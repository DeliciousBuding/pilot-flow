...
export async function buildCallbackVerificationPack({
  cardRunLog = "tmp/runs/latest-card-send.jsonl",
  listenerLog = "tmp/runs/latest-card-listener.jsonl",
  permissionPack = "tmp/demo-permissions/PERMISSION_APPENDIX.md",
...
function buildNextActions({ listener, status }) {
  if (status === "callback_verified") {
    return [
      ...
      "Regenerate Judge Review and Readiness review with callback verified."
    ];
  }
...
function parseArgs(argv) {
  ...
  return {
    ...
    cardRunLog: resolve(typeof args["card-run-log"] === "string" ? args["card-run-log"] : "tmp/runs/latest-card-send.jsonl"),
    listenerLog: resolve(typeof args["listener-log"] === "string" ? args["listener-log"] : "tmp/runs/latest-card-listener.jsonl"),
    permissionPack: resolve(typeof args["permission-pack"] === "string" ? args["permission-pack"] : "tmp/demo-permissions/PERMISSION_APPENDIX.md"),
...
function buildUsage() {
  return `Usage:
  npm run review:callback-verification
