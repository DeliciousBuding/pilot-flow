...
function buildNextActions({ status }) {
  if (status === "needs_regeneration") {
    return [
      ...
      "Rerun the submission pack after readiness and judge review are current."
    ];
  }
  ...
  return [
    ...
    "Rerun `npm run review:submission` with `--capture-manifest` after manual evidence is collected."
  ];
}
...
function buildUsage() {
  ...
  npm run review:submission -- --write-capture-template tmp/demo-submission/capture-manifest.template.json
  npm run review:submission -- --capture-manifest tmp/demo-submission/capture-manifest.json --output tmp/demo-submission/SUBMISSION_PACK.md
