...
| 本地测试 | 35 个测试通过 | `pytest -q` |
| 真实飞书录屏 | 待补齐 | live parity 后补充链接 |
| 真实产物样例 | 待补齐 | live parity 后补充链接 |
...
| 能力 | 当前证据 | 说明 |
| --- | --- | --- |
| 飞书文档/多维表格/任务/入口消息 | 早期真实群 live 路径跑通过 | v1.12 需重新录屏确认 |
| 确认门控 | 本地集成测试 + 代码级 chat_id TTL | 未确认不能创建项目 |
| 卡片确认/取消 | 本地集成测试覆盖 confirm/cancel | 真实按钮点击续跑待 live parity |
| Hermes memory 写入 | 本地集成测试覆盖 dispatch | 默认不持久化成员姓名，可用环境变量开启 |
| Hermes cron 截止提醒 | 本地集成测试覆盖 dispatch | 创建项目时调度截止前提醒 |
| 项目模板/风险检测/看板/多轮更新 | 单元测试 + 集成测试 | 覆盖答辩、sprint、活动、上线模板 |
| LLM 驱动 | Hermes 工具描述 + skill 指令 | 真实 LLM 行为待 live parity 复测 |
...
| 学习能力 | 依赖飞书产品内建能力 | Hermes memory 写入已实现，历史读取和自动建议在下一轮推进 |
