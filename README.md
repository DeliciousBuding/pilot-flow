...
PilotFlow 的 AI 不只是"生成一段文字"，而是驱动整个飞书工作流。
...
技术实现上，PilotFlow 采用 **Agent 架构 + 飞书工具注册表 + 确认门控 + 运行追溯** 四层设计，不是一次性脚本，而是可扩展、可审计的 Agent 运行时。
...
| 步骤 | 手动操作 | PilotFlow |
| --- | --- | --- |
| 整理讨论要点 | 人工翻聊天记录，5-10 分钟 | 自动提取，秒级 |
| 写项目文档 | 打开文档编辑，10-15 分钟 | 自动生成，秒级 |
| 建多维表格状态 | 手动建表、填字段，5 分钟 | 自动写入，秒级 |
| 创建任务 | 逐个创建、分配负责人，5 分钟 | 批量创建，秒级 |
| 发群公告 | 手动编辑、发送、固定 | 自动发送并固定 |
| 记录过程 | 不记录或手动截图 | 自动生成可回放日志 |
| **合计** | **30+ 分钟，5 个工具，容易遗漏** | **一句话触发，全程自动，有记录** |

## 差异化

| 对比维度 | PilotFlow | 通用 AI 助手 | 手动操作 |
| --- | --- | --- | --- |
| 飞书产物 | 直接创建真实文档、表格、任务 | 生成文本，需人工复制粘贴 | 手动逐个创建 |
| 确认机制 | 副作用前必须确认 | 无副作用控制 | 无 |
| 运行追溯 | 每步记录，可回放 | 不可追溯 | 不可追溯 |
| 风险识别 | 自动检测并发起裁决 | 不涉及 | 人工判断 |
| 飞书原生 | 卡片、文档、表格、任务、入口消息一体化 | 无飞书集成 | 分散在各工具 |

可复用性：Agent 架构 + 工具注册表设计，任何团队可以扩展新工具接入，不局限于当前的飞书能力集合。
