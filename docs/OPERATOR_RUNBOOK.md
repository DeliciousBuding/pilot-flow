...
Use `pilot:live-check` before a real write run. It performs read-only checks for `lark-cli`, profile auth, IM event receive scope, chat visibility, Base table visibility, and bot mention identity. It ignores `PILOTFLOW_LLM_*` because this command only validates Feishu live targets.
