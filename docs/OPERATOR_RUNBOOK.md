...
Use `pilot:live-check` before a real write run. It performs read-only checks for `lark-cli`, profile auth, IM event receive scope, IM event subscribe dry-run, card callback subscribe dry-run, callback probe card dry-run, event bus status, chat visibility, Base table visibility, and bot mention identity. It also returns `nextActions` when a check points to a known operator fix. It ignores `PILOTFLOW_LLM_*` because this command only validates Feishu live targets.
