...
- [ ] Configure `PILOTFLOW_BOT_USER_ID`, then verify `im.message.receive_v1` delivery with the gateway probe.
...
3. Set `PILOTFLOW_BOT_USER_ID`, then validate `pilot:gateway -- --live --send-probe-message --timeout 60s --max-events 1 --json`.
