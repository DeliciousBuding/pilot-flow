...
| 项目模板 | 支持 sprint、活动策划、产品上线等常见项目模板，展示参考交付物和工期 |
| Hermes Memory 写入 | 项目创建时 best-effort 保存模式，供后续历史读取使用；默认不保存成员姓名 |
| 卡片回调工具 | 插件级 `/card` 桥接处理确认、取消、标记完成、催办等按钮操作，并更新原卡片状态 |
...
| 历史项目建议 | 读取项目模式，辅助 Agent 建议常用交付物和工期 |
| 项目后端扩展 | 在可用时接入更权威的项目管理后端 |
...
用户在飞书群 @PilotFlow
  ↓
"@PilotFlow 帮我准备产品发布项目空间，成员示例成员A，交付物是发布说明和检查清单，5月7日截止"
  ↓
