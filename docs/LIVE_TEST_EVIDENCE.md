...
| 卡片动作 | 工具输出确认 `briefing_actions_ok=true`、`briefing_batch_reminder_ok=true`、`briefing_risk_followup_task_ok=true`、`briefing_filtered_button_label_ok=true`，说明简报可一键查看风险/逾期项目、批量催办逾期项目，并能按当前筛选批量创建跟进待办；筛选简报的待办按钮会显示“创建风险待办/创建近期待办/创建逾期待办”，普通站会简报仍保持通用按钮 |
