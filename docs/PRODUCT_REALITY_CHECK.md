...
| `pilot:gateway` TS path | Local bridge implemented | It can consume message/card events and resume stored confirmation runs locally through card callbacks or a same-chat `确认执行`. A structured mention probe was sent on 2026-05-01, but no IM event reached the listener within 60 seconds. |
...
- No reliable production-style Feishu group bot loop yet; the structured mention probe can send, but event delivery still times out.
