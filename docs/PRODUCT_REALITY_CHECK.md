...
| `pilot:gateway` TS path | Local bridge implemented | It can consume message/card events and resume stored confirmation runs locally through card callbacks or a same-chat `确认执行`. The IM probe now fails closed unless a real bot `user_id` is configured for structured mentions. |
...
- No reliable production-style Feishu group bot loop yet; the next IM event test needs a configured `PILOTFLOW_BOT_USER_ID`.
