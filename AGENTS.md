...
- WSL Feishu runtime verifier: run `scripts/verify_wsl_feishu_runtime.py` from WSL Hermes runtime with `--env-file ~/.hermes/.env --config-file ~/.hermes/config.yaml`; use `--probe-llm`, `--send-card`, `--verify-history`, `--verify-project-creation`, `--verify-update-task`, `--verify-archive-gate`, `--verify-followup-task`, `--verify-deadline-update`, `--verify-member-permissions`, `--verify-member-removal`, `--verify-risk-cycle`, `--verify-progress-update`, `--verify-project-reminder`, `--verify-briefing-batch-reminder`, `--verify-card-status-cycle`, `--verify-batch-followup-task`, and `--verify-dashboard-navigation` for release evidence.
